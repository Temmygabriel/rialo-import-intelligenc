...
use rialo_worker::{
    WalletSetupConfig, WorkerConfig, DEVNET_KEYRING_NAME,
    DEVNET_MNEMONIC_STRENGTH_BITS,
};
...
async fn main() -> Result<()> {
    match env::args().nth(1).as_deref() {
        ...
        Some("anchor-event") => anchor_event().await,
...
async fn setup_wallet() -> Result<()> {
    ...
    std::fs::create_dir_all(&config.keyring_path).with_context(|| {
        format!(
            "failed to create Rialo keyring directory {}",
    ...
    let confirmation =
        read_password_once("Confirm password for new Rialo DevNet keyring: ")?;
    ...
    let (created_keyring, mnemonic) = provider
        .create_with_mnemonic(
            &config.keyring_name,
            DEVNET_MNEMONIC_STRENGTH_BITS as u32,
            &password,
        )
        .await
    ...
    if loaded_pubkey != created_pubkey {
        return Err(anyhow!(
            "created keyring pubkey did not match reloaded keyring pubkey"
        ));
    }
    ...
    println!(
        "RECOVERY MNEMONIC - SAVE THIS NOW; IT WILL NOT BE WRITTEN TO DISK:"
    );
    println!("{mnemonic}");
    println!(
        "RECOVERY: keep the encrypted keyring file, this password, and the mnemonic in a secure vault. Later load this wallet with FileKeyringProvider::new(KEYRING_DIRECTORY).load(\"{DEVNET_KEYRING_NAME}\", password). Use recover_from_mnemonic only if the encrypted keyring file is lost."
    );
...
fn read_password_once(prompt: &str) -> Result<String> {
    ...
    io::stdin()
        ...
        .context("failed to read keyring password")?;
...
async fn anchor_event() -> Result<()> {
    let event = env::var("RIALO_EVENT_JSON")
        .context("RIALO_EVENT_JSON is required")?;

    if event.trim().is_empty() {
        return Err(anyhow!("RIALO_EVENT_JSON must not be empty"));
    }
    ...
    let provider =
        FileKeyringProvider::new(Path::new(&config.keyring_path));
    ...
    let recipient = Pubkey::from_str(&config.recipient)
        .context("RIALO_RECIPIENT must be a valid Rialo public key")?;
    ...
    .add_transfer_instruction(
        &payer,
        &recipient,
        config.amount_kelvin,
    )
    .sign(&keyring)
    .context("failed to build and sign anchor transaction")?;

    let confirmation = client
        .send_and_confirm_transaction(&signed_transaction, None)
        .await
        .context("failed to submit and confirm Rialo anchor transaction")?;

    let signature =
        rialo_cdk::generated::types::Signature::from_str(
    ...
    let output = WorkerOutput {
        ...
        signature_statuses: serde_json::to_value(signature_statuses)
            .unwrap_or_else(|err| {
                serde_json::json!({
